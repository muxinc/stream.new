...
const WinampPlayer: React.FC<Props> = ({ playbackId, poster, currentTime, onLoaded }) => {
  useEffect(() => {
    onLoaded();
  }, []);
    ...
    <>
      ...
      <style jsx>{`
        ...
        .winamp-player :global(media-volume-range:hover) {
          ...
          --media-range-thumb-height: 10px;
          --media-range-thumb-border-radius: 0;
          --media-range-thumb-background: 53px 443px url(/winamp-player/BALANCE.BMP);
        ...
        .window .bottom {
          width: 100%;
          background-image: url(/winamp-player/GEN.BMP), url(/winamp-player/GEN.BMP);
