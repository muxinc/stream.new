...
export async function POST(request: NextRequest) {
  ...
  if (webhookSignatureSecret) {
    ...
    try {
      event = await mux.webhooks.unwrap(rawBody, headers, webhookSignatureSecret);
    } catch (e) {
      console.error('Error unwrapping webhook - is the correct signature secret set or payload incorrect?', e);
